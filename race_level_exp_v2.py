"""
race_level_exp_v2.py — レースレベル(強さ重み付け版)の A/B (市場非経由・印精度のみ)
================================================================================
arm A = v6 120特徴 (baseline 再学習) / arm B-W2,B-W3 = A + race_level_str_W{2,3}
集約=「好走の強さ」加重(着順0.3/着差0.3/補正タイム0.4)版。harnessは v1 実験と同一。
arm A は v1 実験(◎top3=61.48%)を再現するか確認。
出力: reports/race_level_exp_v2.json
実行: PYTHONUTF8=1 python race_level_exp_v2.py
"""
...
PARQUET = BASE / "data/race_level_feats_v2.parquet"
OUT_JSON = BASE / "reports/race_level_exp_v2.json"
SEED = 42
...
NEW_BY_WINDOW = {"W2": ["race_level_str_W2"], "W3": ["race_level_str_W3"]}
ALL_NEW = ["race_level_str_W2", "race_level_str_W3"]
HELP_GROUPS = {
...
def load_master():
    ...
    cov = {sp: round(df.loc[df["split"] == sp, "race_level_str_W3"].notna().mean() * 100, 1)
           for sp in ["train", "valid", "test"]}
    log(f"  rows={len(df):,}  level(str) W3 カバレッジ={cov}")
    return df, cov
...
def main():
    ...
    results = {"protocol": "v6params固定 / train≤2022 / valid2023 early-stop / test2024-25 / 市場非経由",
               "note": "集約=好走の強さ加重(着順0.3/着差0.3/補正タイム0.4)。前走レベルのみ(近走平均未実装)。",
               "compare_v1": {"marks_top3_A": 0.6148, "v1_best_top3_B": 0.6161, "v1_best_window": "W2",
                              "v1_top_new_feat_rank": 11}}
    df, cov = load_master()
    ...
    if best_d >= 0.005 and top_rank <= 40:
        verdict = (f"TRANSLATED: B_{best_win}の◎top3が A比 {best_d*100:+.2f}pt 上昇かつimportance上位({top_rank}位)。"
                   "集約の精緻化で情報が精度に翻訳された。反復整合版へ進む価値が確定。")
    elif top_rank <= 40 and best_d < 0.005:
        verdict = (f"REDUNDANT: importance上位({top_rank}位)だが◎top3 A比 {best_d*100:+.2f}pt(誤差内)。"
                   "★レベル情報は既存特徴と冗長=独自情報が薄い。集約を精緻化しても天井不変。能力解明の幹も精度面でここで閉じる。")
    else:
        verdict = (f"WEAKER_IMP: importance {top_rank}位 (v1の11位より低下なら重み付け逆効果)。"
                   f"◎top3 A比 {best_d*100:+.2f}pt。集約定義の再検討。")
    results["verdict"] = verdict
    results["summary"] = {
        "marks_top3_A": a3,
        "marks_top3_B": {w: results[f"B_{w}"]["marks_test"]["hon_top3"] for w in NEW_BY_WINDOW},
        "ndcg5_A": results["A"]["marks_test"]["ndcg5"],
        "ndcg5_B": {w: results[f"B_{w}"]["marks_test"]["ndcg5"] for w in NEW_BY_WINDOW},
