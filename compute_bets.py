...
# [CI -9.7,-5.8] 縮む(steam)。単勝EVの期待払戻 = p_win × E[確定|勝ち] なので T-10 オッズ
# 素のEVは系統的過大計上。バンド別に補正 (1倍台は CI が 1 を跨ぐため 1.0 に丸め、
# n<60 帯は全体平均で平滑)。前提監査(2026-07-30)の「15-30倍帯 符号逆」は n=43 の
# ノイズと確定 (再fit: ×0.988 CI[0.89,1.10])。8-20帯は ×0.935 CI[0.889,0.982] で有意。
# 20+帯は n=39<60 → 全体平均 0.922 (実測点 0.970 CI[0.87,1.08]、次回再fitで要再確認)。
# 表示・買い目オッズは生のまま、EV(選別・配分)にのみ適用。
# 鮮度管理: weekly_post が settle_drift.json の鮮度をチェックし月次で再実測を促す。
SETTLE_DRIFT_TAN = [(2.0, 1.00), (4.0, 0.935), (8.0, 0.852), (20.0, 0.935), (9e9, 0.922)]
# 複勝 (n=1369, ×0.885 CI[0.872,0.900]) / ワイド (n=1387, ×0.920 CI[0.909,0.932])
