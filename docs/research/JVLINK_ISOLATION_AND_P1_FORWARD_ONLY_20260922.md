...
> ## ⚠ ERRATUM（2026-09-22、`P1_FORWARD_ONLY_PHASE_AB_20260922.md` §0 により訂正）
>
> 本書が前提にしていた「`data/weekly` は障害競走を含まない（意図的 scope filter）」
> は**誤り**。実際には**日によって不安定**で、保存済み 8 日中 4 日で障害が weekly に
> 入っており、**3 レースが production bundle で v6 に採点・印付けされていた**
> （実購入は 0 円、ただしコードレベルの拒否は存在しなかった）。
> 2026-09-22 に P0 hard gate を 5 層へ実装して再発を防止した。
> §1 の項目 2「weekly live prediction/betting は障害を対象外にしている」は、
> **P0 gate 実装後に初めて成立する**記述である（実装前は成立していなかった）。
