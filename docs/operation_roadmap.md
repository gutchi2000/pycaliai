...
`weekly_nicegui.ps1` は **3 フェーズ** を 1 つのスクリプトに統合:

| Phase | フラグ | タイミング | 役割 |
|---|---|---|---|
| A | (default) | 土曜朝 | TARGET CSV 配置後、bundle.json 生成 + push |
| B | `-BetsOnly` | 土曜午前 | Cowork が返した買い目 JSON を push |
| C | `-Post` | 日曜夜 | weekly_post.ps1 + HF 同期 |

### Phase A: 土曜朝 (レース前)

TARGET から CSV エクスポート → `data\weekly\{date}.csv` 等を配置してから:
...
4. `git add` → `git push origin HEAD:master` (Streamlit Cloud 反映)
5. `sync-hf.ps1` (HF Spaces / NiceGUI 反映)

この時点で NiceGUI は **出走表 + 全頭分析** までは見られる。買い目タブはまだ空。

### Phase B: 土曜午前 (Cowork 買い目を反映)

```text
1. bundle.json (reports\cowork_input\{date}_bundle.json) を
   Cowork (Anthropic Desktop App) のチャットに投げる
2. Cowork が race ごとに買い目 JSON を返す
3. その JSON を保存:
...
```powershell
.\weekly_nicegui.ps1 -BetsOnly                # 当日自動判定
.\weekly_nicegui.ps1 20260502 -BetsOnly       # 日付指定
```

実行内容:
1. `reports\cowork_output\{date}_bets.json` の存在確認
2. `git add reports/cowork_output` → `git push origin HEAD:master`
3. `sync-hf.ps1` で HF に反映

NiceGUI の `🎫 Cowork 買い目` タブで race ごとの買い目が見えるようになる。

### Phase C: 日曜夜 (レース後)

```powershell
.\weekly_nicegui.ps1 -Post                    # 当日自動判定
.\weekly_nicegui.ps1 20260502 -Post           # 日付指定
```

実行内容:
1. `weekly_post.ps1 {date}` (既存スクリプト) → `data/results.json` 更新 + GitHub push
2. `sync-hf.ps1` (任意、HF にも反映)

NiceGUI は当面 results.json を表示しないので、Streamlit 側でのみ確認用。
...
.\weekly_nicegui.ps1
  ...
  → NiceGUI で 出走表 + 全頭分析 が見られる。Cowork 買い目タブは空。

【Phase B : 土曜午前】Cowork 買い目を反映
人手:
  1. reports/cowork_input/{date}_bundle.json を Cowork (Anthropic Desktop) に投入
  2. Cowork が買い目 JSON を返す
  3. reports/cowork_output/{date}_bets.json として保存

.\weekly_nicegui.ps1 -BetsOnly
  ├─ git add reports/cowork_output/
  ├─ git push origin master  → Streamlit Cloud
  └─ sync-hf.ps1             → HuggingFace Spaces (NiceGUI)

  → NiceGUI の Cowork 買い目タブが埋まる。
...
.\weekly_nicegui.ps1 -Post
  ...
  │   └─ git push origin master  → Streamlit Cloud
  └─ sync-hf.ps1                  → HuggingFace Spaces (任意)
```
