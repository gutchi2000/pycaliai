...
# 戦略重みの元データは valid セット（2023年）を使う
# train（2013-2022）はモデルが学習済みのため ROI が過楽観
_csv = r'E:\PyCaLiAI\reports\backtest_results_valid.csv'
if len(sys.argv) > 1 and sys.argv[1] == '--train':
    _csv = r'E:\PyCaLiAI\reports\backtest_results_train.csv'
    print('[INFO] trainデータを使用（参考用）')
else:
    print('[INFO] validデータを使用（推奨）')
