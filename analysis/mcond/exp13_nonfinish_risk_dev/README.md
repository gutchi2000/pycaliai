...
7. **欠損・選択バイアス**: 非完走馬はv6の学習・評価母集団の外側に構造的に
   ...
   **⚠ ERRATUM 2026-09-22: この記述は誤り。** `芝・ダ`列で判定したための
   誤りで、確定判定の`トラックコード(JV)`(51-59=障害)では **v6のtrain split
   に障害14,716行/1,267レース(3.03%)が実際に含まれている**。v6の120特徴には
   `トラックコード(JV)`自体も入っている。詳細は`DATA_AUDIT.md`§1/§3の
   ERRATUM、および
   `docs/research/JUMP_RACE_CONTRADICTION_AND_ASOF_IMPACT_20260922.md`。
