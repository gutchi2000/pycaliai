...
**Stage 0 完了・終了（2026-09-25、Stage 0 成果物 commit `ed6960a1`）。事前登録した停止規律 1（EXP02 scalar ability との等価）と 2（被覆が凍結床未満）に該当。Stage 1 以降は実施しない。**
...
- **E0-B（2022 label-free 実測）**: 未対戦 pair の `d_ij − b·Δμ_ij` の分散は、EXP02 能力から各対戦を再抽選した null の **0.840 倍**（CI95 [0.825, 0.856]、事前固定 FAIL 条件 上限 < 1.10）。事前固定した超過分散基準では、pair 固有状態は null 抽選ノイズを上回らなかった。
...
- 閉じたのは **EXP12 で検証した 1-hop 表現と、EXP17 で検証した時点安全な共通対戦 2-hop 表現＋Hodge スカラー射影を閉じた**。対戦相手 identity 系全体を閉じたとは主張しない。
- **s と μ の差は、事前固定した超過分散基準では null 抽選ノイズを上回らず、追加の pair 固有状態として Stage 1 へ進める根拠にならなかった**。
