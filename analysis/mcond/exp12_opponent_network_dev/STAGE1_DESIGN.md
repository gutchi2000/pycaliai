...
**作成日**: 2026-09-21　**状態**: **完全終了**（O3がGateで不合格、O4以降は未実装）。
2024・2025年の性能・ROIは一切開封していない。

## 結論（先出し）

**主仮説（O3がO2を超える固有価値を持つか）は支持されなかった。** 特に
placebo検定（層内での交絡だけで同等以上の"改善"が再現するか）が決定的に
不合格であり、観測された小さな改善（logloss -0.00040）の大部分が年齢×
経験数という交絡層で説明できてしまうことを直接示した。ユーザーの事前
指示「O3がFAILなら『単純だったから高度化すればよい』と解釈せず、O4〜O6を
実装しない」に従い、EXP12をここで終了する。
...
**実施しない**。O3がGateで不合格のため、2-hop/PageRank(O4)・graph
embedding(O5)・GNN(O6)のいずれも実装しない。「単純だったから高度化
すればよい」という解釈はしない（ユーザー事前指示通り）。
...
- Placebo検定（層内交絡のみでの再現性検定）が**決定的にFAIL**——観測
  された小さな改善は交絡で説明可能であり、対戦相手の個体識別情報が
  O2を超える固有価値を持つという主仮説を支持しない。
- meeting-day forward-chaining CVでも開催日bootstrap CIの上限がわずかに
  ゼロを超え、続行条件を満たさない。
- 事前登録した停止規律に従い、**O4以降(2-hop/PageRank/embedding/GNN)は
  実装せず、2024・2025年を一切開封せずEXP12を終了する**。
