...
## 13. Stage 0結果（2026-09-25、定義は変更せず追記のみ）

| 監査 | 結果 |
|---|---|
| S0-A 先行研究・等価性 | 同値な先行実施なし（T0/T1頭対頭は`crux_joint.py`で既実施→anchor専用）。停止規律1は非該当 |
| S0-A anchor（9時・≤2018） | **PASS**。λ*=1.1、Δ=LL(Harville)−LL(市場)=**+0.0416**（市場優位、要求範囲0.0185〜0.074） |
| S0-B provenance | **PASS**。TANPUK/UMAREN 2013〜2023、区分1/4、`(rid16,区分,月日時分)`完全一致結合。無接頭辞227列ファイルは不採用 |
| S0-C exactness | 合成oracle/invariant **33/33 PASS** |
| S0-C 複勝往復 | **FAIL**（各年馬行合格率0.02〜0.27%、要求99%）→ **T2未実装で停止、UB3はGate armから除外**。Gate armはUB2の1本 |
| S0-D coverage | **PASS**。2019〜2023の16,644 race、terminal・全組合せ完全・pre/terminal同時とも**1.000** |
| S0-E 実務床 | **0.8843 nats/race**（結果開封前にcommit `ee5b585a`） |
| S0-E 検出力 | **PASS**。床でのSIGNAL検出力**1.000**（400/400、Wilson95 [0.990,1.000]）、0.5倍・2倍でも1.000。ただし床での推定中央値は0.742（減衰0.84）で床を下回る |
...
UB2 の pooled Δ = −0.001445、CI95 [−0.002359, −0.000532]、4/5 年で負。実 Δ はP1/P2のnull分布の2.5%分位より小さく、改善側97.5 percentileを超えた。（real Δ = -0.001445 / P1 q025 = -0.000062 / P2 q025 = -0.000058）。ただし 2021 年または 2022 年を除く LOO で
