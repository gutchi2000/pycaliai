...
## 12. 過去の mcond 評価を読むときの注記（DNF の市場再正規化）

改訂2 の実測から、**過去研究の数値を再解釈するときの注意点**が一つ出たので prior-art として記録する。

- **平地 DNF ありレースでは、finisher だけへ市場を再正規化すると Q0 の logloss が約 0.061 nats 有利に見える**
  （2016-2022 の該当 937 レースで starter 全頭正規化 2.045111 対 finisher 再正規化 1.983745、差 **+0.061366**）。
  完走しなかった馬に割り当てられていた確率を勝馬側へ配り直すため、市場が実際より当たっているように見える。
- 平地全体（23,299 レース）で平均すると **約 0.00247 nats/race**（DNF ありは 4.02% のレース）。
  これは今回の実務床 **0.005 nats/race の 49.4%**、つまり**約半分**に相当する規模である。
- これは **過去の mcond 評価を再解釈する際の注意事項**であり、
  **それだけで過去実験の結論を覆すものではない**。過去実験の主判定はモデル対モデルの比較や
  ROI 判定であり、市場 logloss の水準そのものを結論の根拠にしていたわけではない。
  どの実験がどの正規化を使っていたかは、必要になった時点で個別に確認する。
- **EXP16A はこの経路を回避する**。正式 race set を**平地・DNF なし**に限定し、starter == finisher が
  成り立つ集合だけで評価する（`RACE_POPULATION_AUDIT.md` §4-5）。DNF ありレースは感度分析でのみ報告する。
