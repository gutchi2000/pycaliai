...
# Phase 5 新モデルで◎複勝率を再検証した結果、阪神(58.2%) 京都(60.1%)は
# 新潟(56.6%) 福島(55.9%)より上のため全面除外を解除 (2026-04-05)
# 旧根拠: 阪神57.9%/京都12.9%は旧モデルのbacktest値であり新モデルでは改善済み
SKIP_VENUES: set[str] = set()  # 全面除外なし

# 16頭立てのみ除外する競馬場
# 阪神16頭:51.9%, 京都16頭:55.5%, 新潟16頭:元々除外
...
# NOTE: Phase 5 で新アンサンブル（7モデル最適化重み）に移行したが、
#       キャリブレーター(v4)は旧モデル用のため ev_cal が 0.72-0.78 に圧縮される。
#       calibrate.py 再実行後に 0.78 に戻すこと。(2026-04-05)
EV_CAL_LOWER_LIMIT: float = 0.70
