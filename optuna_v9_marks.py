...
optuna_v9_marks.py
==================
Phase 1 (印精度向上): v6 をベースに「上位 3 位に集中」する変更を加えた版。

v6 との差分:
  1. lambdarank_truncation_level: 5 → 3
     LambdaRank の損失計算を上位 3 位だけで打ち切り。 ◎〇▲ の順位精度に集中。
  2. composite weight 再配分: NDCG@5 → NDCG@3 + 上位印重視
     v6:
       W_NDCG5            = 0.30
       W_HON_TOP3         = 0.25
       ...
       W_HON_TOP2         = 0.10
     v9:
       W_NDCG3            = 0.40   (NDCG@5 → NDCG@3)
       W_HON_TOP3         = 0.30
       W_HON_TOP2         = 0.20
       W_TOP3_SUBSET_TOP3 = 0.10   (top5 subset → top3 全部当たり)
       (winner_in_top5 廃止、 上位 3 位の精度に集中)

実行: python optuna_v9_marks.py [--n-trials 40]

出力:
  models/unified_rank_v9.pkl
  reports/optuna_v9_marks.json
"""
...
OUT_MODEL  = BASE / "models/unified_rank_v9.pkl"
OUT_JSON   = BASE / "reports/optuna_v9_marks.json"
...
# v9: 上位 3 位に集中した composite weight
W_NDCG3            = 0.40
W_HON_TOP3         = 0.30
W_HON_TOP2         = 0.20
W_TOP3_SUBSET_TOP3 = 0.10
# v6 の ECE penalty は維持
W_ECE_PENALTY      = 0.5
...
def ndcg_at_k(label_arr, score_arr, k=3):
    n = len(label_arr)
    if n < k: k = n
...
def evaluate_marks(vl_scored, p_threshold=0.10):
    """v9: NDCG@3, ◎top3, ◎top2, top3 subset of top3 (1-3着全部 ◎〇▲ にマッチ) を測定."""
    n_race = 0
    ndcg3_sum = 0.0
    hon_top3 = 0
    hon_top2 = 0
    top3_subset_top3 = 0  # 1-3 着が全部 ◎〇▲ にマッチ
    p_high_all = []
    actual_high_all = []
    ...
    for rid, g in vl_scored.groupby(COL_RID, sort=False):
        ...
        ndcg3_sum += ndcg_at_k(rel, scores, k=3)
        ...
        if actual_top3 == top3_pred: top3_subset_top3 += 1
    ...
    return {
        "n_race": n_race,
        "ndcg3": ndcg3_sum / n_race,
        "hon_top3_rate": hon_top3 / n_race,
        "hon_top2_rate": hon_top2 / n_race,
        "top3_subset_top3_rate": top3_subset_top3 / n_race,
        "ece_high_p": ece_high,
    }
...
def composite_score(m):
    base = (
      ...
      + W_TOP3_SUBSET_TOP3 * m["top3_subset_top3_rate"]
    )
...
def objective(trial):
    ...
    params = {
        ...
        "lambdarank_truncation_level": 3,           # v9: 5 → 3
        "metric":                     "ndcg",
        "eval_at":                    [3],
    ...
    for k in ["ndcg3", "hon_top3_rate", "hon_top2_rate",
              "top3_subset_top3_rate", "ece_high_p"]:
        trial.set_user_attr(k, metrics[k])
    print(f"  trial {trial.number:>2}: composite={composite*100:6.3f}  "
          f"ndcg3={metrics['ndcg3']:.4f}  ◎top3={metrics['hon_top3_rate']*100:5.2f}%  "
          f"◎top2={metrics['hon_top2_rate']*100:5.2f}%  "
...
def main():
    ...
    args = ap.parse_args()
    print("=" * 80)
    print(f"optuna_v9_marks.py  (n_trials={args.n_trials}, n_folds={N_FOLDS}, seed={SEED})")
    print(f"  Phase 1: lambdarank_truncation_level=3 + composite を上位 3 位重視に")
    print("=" * 80)
    ...
    joblib.dump({
        ...
        "truncation_level": 3,
        "composite_weights": {
            "W_NDCG3": W_NDCG3, "W_HON_TOP3": W_HON_TOP3,
            "W_HON_TOP2": W_HON_TOP2,
            "W_TOP3_SUBSET_TOP3": W_TOP3_SUBSET_TOP3,
        },
        "description": "unified_rank_v9: v6 base + LambdaRank trunc=3 + 上位 3 位重視 composite。 "
                       "印精度向上が目標 (Phase 1 of 印精度改善試行).",
    }, OUT_MODEL)
    ...
    with open(OUT_JSON, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "best_metrics": {k: study.best_trial.user_attrs[k] for k in
                             ["ndcg3", "hon_top3_rate", "hon_top2_rate",
                              "top3_subset_top3_rate", "ece_high_p"]},
            "label_scheme": "clip(6 - 着順, 0, 5)",
            "truncation_level": 3,
            "composite_weights": {
